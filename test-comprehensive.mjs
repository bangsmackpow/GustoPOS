...
async function testManager() {
  ...
  try {
    const loginRes = await makeRequest("POST", "/api/admin-login", {
      username: "GUSTO",
      password: process.env.ADMIN_PASSWORD || "0262",
    });
...
async function testEmployee() {
  ...
  let orderId = "";
  try {
    // First, get a drink ID from the list
    const drinksRes = await makeRequest("GET", "/api/drinks", null, {
      Cookie: employeeCookie,
    });

    const drinkId = drinksRes.body[0]?.id || "test-drink-1";
  ...
  if (tabId) {
    try {
      const tabDetailsRes = await makeRequest(
        `GET`,
        `/api/tabs/${tabId}`,
        null,
        {
  ...
  if (orderId) {
    try {
      ...
      logTest(
        ...
        passed,
        `${discountRes.body.discountMxn || 0} MXN discount`,
      );
    } catch (err) {
