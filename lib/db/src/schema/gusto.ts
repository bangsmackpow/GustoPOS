...
// NOTE: ingredientsTable is being phased out in favor of inventoryItemsTable in inventory.ts
...
export const staffShiftsTable = sqliteTable("staff_shifts", {
  ...
  staffUserId: text("staff_user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  clockInAt: integer("clock_in_at", { mode: "timestamp" }).notNull().default(sql`(unixepoch())`),
