...
async function _getReportData(shiftId: string) {
  ...
  const cashSalesMxn = closedTabs
    .filter((t) => t.paymentMethod === "cash")
    .reduce((sum, t) => sum + Number(t.totalMxn), 0);
  const cardSalesMxn = closedTabs
...
router.get("/shifts", async (req: Request, res: Response) => {
  ...
  const users =
    userIds.length > 0
      ? await db
          ...
          .from(usersTable)
          .where(
            sql`${usersTable.id} IN (${sql.raw(userIds.map((id) => `'${id}'`).join(","))})`,
          )
      : [];
  ...
  return res.json(
    shifts.map((s) => formatShift(s, userMap.get(s.openedByUserId))),
  );
});
...
router.post("/shifts", async (req: Request, res: Response) => {
  ...
  const [user] = await db
    .select()
    .from(usersTable)
    .where(eq(usersTable.id, openedByUserId));
  const userName = user
...
router.get("/shifts/active", async (req: Request, res: Response) => {
  ...
  const [user] = await db
    .select()
    .from(usersTable)
    .where(eq(usersTable.id, shift.openedByUserId));
  const userName = user
    ? `${user.firstName ?? ""} ${user.lastName ?? ""}`.trim() ||
      user.email ||
      user.id
  ...
  const openTabs = await db
    ...
    .where(
      and(
        ...
        ne(tabsTable.status, "closed"),
      ),
    );

  if (openTabs.length > 0 && !force) {
    return res.status(400).json({
      error: "Cannot close shift with open tabs",
      openTabs: openTabs.map((tab) => ({
        id: tab.id,
        nickname: tab.nickname,
        totalMxn: tab.totalMxn,
      })),
    });
  }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    const drinks =
      allDrinkIds.length > 0
        ? await db
            .select()
            .from(drinksTable)
            .where(
              sql`${drinksTable.id} IN (${sql.raw(allDrinkIds.map((id) => `'${id}'`).join(","))})`,
            )
        : [];
    ...
    const recipeRows =
      allDrinkIds.length > 0
        ? await db
            .select({ ri: recipeIngredientsTable, inv: inventoryItemsTable })
            ...
            .where(
              sql`${recipeIngredientsTable.drinkId} IN (${sql.raw(allDrinkIds.map((id) => `'${id}'`).join(","))})`,
            )
        : [];
    ...
    for (const { ri, inv } of recipeRows) {
      ...
      drinkRecipeMap
        .get(ri.drinkId)!
        .push({ ingredientId: ri.ingredientId, amountInBaseUnit: amt });
    }
    ...
    >();
    const uniqueStaffIds = [...new Set(closedTabs.map((t) => t.staffUserId))];
    const staffUsers =
      uniqueStaffIds.length > 0
        ? await db
            .select()
            .from(usersTable)
            .where(inArray(usersTable.id, uniqueStaffIds))
        : [];
    ...
    for (const tab of closedTabs) {
      if (!staffMap.has(tab.staffUserId)) {
        const name = staffUserMap.get(tab.staffUserId) || tab.staffUserId;
        staffMap.set(tab.staffUserId, {
          name,
          totalSales: 0,
          tabsClosed: 0,
          drinksServed: 0,
    ...
    for (const order of allOrders) {
      ...
      if (!categorySalesMap.has(category))
        categorySalesMap.set(category, { qty: 0, sales: 0 });
      const cs = categorySalesMap.get(category)!;
      cs.qty += order.quantity;
      cs.sales += total;
    }

    const salesByDrink = [...drinkSalesMap.entries()]
      .map(([drinkId, v]) => ({
        drinkId,
        drinkName: v.name,
      ...
      .sort((a, b) => b.profitMxn - a.profitMxn);

    const topSellers = [...salesByDrink]
      .sort((a, b) => b.quantitySold - a.quantitySold)
      .slice(0, 10);

    const salesByCategory = [...categorySalesMap.entries()].map(
      ([category, v]) => ({
        category,
        quantitySold: v.qty,
        totalSalesMxn: v.sales,
    ...
    const inventoryUsedMap = new Map<
      string,
      { name: string; baseUnit: string; amount: number; currentStock: number }
    >();
    ...
    const inventoryUsed = [...inventoryUsedMap.entries()].map(
      ([ingredientId, v]) => ({
        ingredientId,
